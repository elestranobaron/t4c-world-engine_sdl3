...
FontManager::get_text(char *text)
{
 ...
 //Mesure les dimensions du texte
  ...
  //Cr�e l'image
  SDL_Surface *sdltext;
  sdltext = TnC_CreateRgbaSurface(largeur_texte, nb_lignes * (font->h + 2));
...
FontManager::get_text(char *text, unsigned int color)
{
  ...
  const Uint32 mapped = TnC_MapArgb(srf, color);

  for (int i = 0; i < srf->h; i++)
   for (int j = 0; j < srf->w; j++)
     if (ptr[i*srf->w + j])
       ptr[i*srf->w + j] = mapped;
